...
> **特許庁の公式「特許情報取得 API」を MCP サーバー化して、Claude Desktop / Claude Code / iPhone Claude から自然言語で日本特許を引く。**
...
### なぜキーワード検索を別カテゴリに分離するのか

公式 JPO API は **番号ルックアップ型のみ**で、各エンドポイントは出願 / 公開 / 登録番号、出願人コード、または **完全一致**の出願人名を取る。キーワード / IPC / F ターム / 日付レンジ / 部分名検索は仕様に存在しない。よって:

- `tools_official/` — 名前は `jpo_*` 始まり、レスポンスは `{"source": "jpo_official", …}`
- `tools_external/` — 名前は `external_*` 始まり、レスポンスは `{"source": "google_patents_unofficial", …}`
- boundary test が `tools_external/` から `tools_official/` への `import` を一切禁止。**サイレントフォールバックなし** — 非公式ソースを参照するかは LLM が判断する。
...
services:
  ip-mcp:
    ports:
      - "YOUR_SERVER_IP:8765:8765"   # 自分の LAN IP
```
...
SSE クライアントでは `/sse` を登録します。Codex direct HTTP と SSE クライアントを同じ公開サーバーで併用する場合は、`MCP_TRANSPORT=both` で起動すると `/mcp` と `/sse` の両方を同じ OAuth 設定で提供できます。単一クライアントだけなら `MCP_TRANSPORT=sse` (default) または `MCP_TRANSPORT=streamable-http` も利用できます。

### iPhone Claude / claude.ai (公開、OAuth 2.1)

公開には Cloudflare Tunnel + Caddy (CF Origin Cert) 構成を推奨します（ホームルーターのポート開放不要、ヘアピン NAT 問題も発生しない）。Let's Encrypt + 直接 443 公開でも動きます。いずれの場合も `MCP_OAUTH_MASTER_PASSWORD` + `MCP_OAUTH_ISSUER_URL` をセットして OAuth 2.1 (DCR + PKCE + マスターパスワード認可) を有効化します。クライアントトークンは SQLite に永続化されコンテナ再起動でも生き残ります。
...
- `jpo_fetch_full_record` は内部で **4 つの公式エンドポイントを並列**で叩くため、1 コール = 4 つの別々の日次クォータから 1 ずつ消費する（同一クォータから 4 ではない）。ボトルネックは最も少ないクォータ。
...
| プレースホルダ | 例 | 設定方法 |
|---|---|---|
| `YOUR_SERVER_IP` | `192.0.2.10` | デプロイ先サーバーの LAN IP |
| `<SSH_USER>` | `youruser` | サーバーの SSH ユーザー名 |
| `your-host.example.com` | 自分のドメイン | Cloudflare / リバースプロキシ背後の公開ホスト名 |
